<?php 
	...
	class Usuarios extends Controllers{
		...
		public function Usuarios()
		{
			...
			$data['page_title'] = "USUARIOS <small>MEGA BIT</small>";
			...
			if($_POST){
				...
				if(empty($_POST['txtIdentificacion']) || empty($_POST['txtNombre']) || empty($_POST['txtApellido']) || empty($_POST['txtTelefono']) || empty($_POST['txtEmail']) || empty($_POST['listRolid']) || empty($_POST['listStatus']) )
				{
					...
					if($request_user > 0 )
					{
						...
						$arrResponse = array('status' => false, 'msg' => '¡Atención! el correo electrónico o el CUI ya existe, ingrese otro.');		
					}else{
		...
		public function getUsuarios()
		{
			if ($_SESSION['permisosMod']['r']) {
				...
				for ($i=0; $i < count($arrData); $i++) {
					...
					{
						$arrData[$i]['status'] = '<div class="text-center"><span class="badge badge-success">Activo</span></div>';
					}else{
						$arrData[$i]['status'] = '<div class="text-center"><span class="badge badge-danger">Inactivo</span></div>';
					}
					if($_SESSION['permisosMod']['r']){
